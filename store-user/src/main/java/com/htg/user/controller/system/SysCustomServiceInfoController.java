...
public class SysCustomServiceInfoController {
    ...
    @ApiOperation(value = "管理员直接添加客服")
    @ResponseBody
    @PostMapping("/custom_service/add")
    public CommonResult<RespId> addCustomServiceInfo(@Valid @RequestBody CustomServiceUserDTO serviceUserDTO) throws GlobalException {
        return customServiceInfoService.addCustomService(serviceUserDTO);
    }
    ...
    public CommonResult<RespPage<CustomServiceUserInfoVo>> searchCustomService(@Valid @RequestBody CustomSearchDto customSearchDto) {
        ...
        String realName = customSearchDto.getRealName();
        if (realName != null) {
            realName = "%" + realName + "%";
        }
        String tel = customSearchDto.getTel();
        log.info("========>name is  {},tel is {}", serviceName, tel);
        return customServiceInfoService.searchCustomSerivce(page, tel, serviceName,realName);
    }
