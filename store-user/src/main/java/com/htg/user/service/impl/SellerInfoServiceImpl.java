...
public class SellerInfoServiceImpl extends ServiceImpl<SellerInfoMapper, SellerInfo> implements ISellerInfoService {
    ...
    public CommonResult verifySellerInfo(SellerVerifyDto verifyDto) throws GlobalException {
        ...
        if (state == SellerConst.STATE_VERIFY_UNPASS) {
            // 审核通过必须分配客服
            if (verifyDto.getCusServiceId() == null) throw new GlobalException(CodeEnum.MUST_HAVE_CUSTOM_SERVICE);
            // 有客服id 要做下检查是否真的存在
            ...
            // 对机构进行检测
            if (verifyDto.getAgencyId() == null) throw new GlobalException(CodeEnum.MUST_HAVE_AGENCY);
            //todo 检测机构是否已经存在
        }
    ...
    public CommonResult<RespId> addSysSeller(SysSellerAddDto sysSellerAddDto) throws GlobalException {
        ...
        /* 对客服信息进行检查 */
        // 审核通过必须分配客服
        if (sellerInfo.getCusServiceId() == null) throw new GlobalException(CodeEnum.MUST_HAVE_CUSTOM_SERVICE);
        // 有客服id 要做下检查是否真的存在
